 ...
 */
String.prototype.score=function(l){if(this===l){return 1}var e=0,p=l.length,f=this,o=f.length,n=0,j,s,r,h;for(var d=0,q,m,g,a,b,k;d<p;++d){g=l[d];a=f.indexOf(g.toLowerCase());b=f.indexOf(g.toUpperCase());k=Math.min(a,b);m=(k>-1)?k:Math.max(a,b);if(m===-1){return 0}q=0.1;if(f[m]===g){q+=0.1}if(m===0){q+=0.8;if(d===0){n=1}}if(f.charAt(m-1)===" "){q+=0.8}f=f.substring(m+1,o);e+=q}j=e/p;s=p/o;r=j*s;h=(r+j)/2;if(n&&(h+0.1<1)){h+=0.1}return h};